...
router.get('/', function(req, res, next) {
  res.send('basic instructions on how to use API here... obvious placeholder is obvious...');
});
...
router.get('/marsCoords/latitude/:latitude/longitude/:longitude', function (req, res) {
  ...
  // validate latitude
  if (latitude < -90) {
    error.latitude = `Latitude is below (-90 to 90) at ${latitude}`;
  }
  else if (latitude > 90) {
  ...
  // validate longitude
  if (longitude < -180) {
    error.longitude = `Longitude is below (-180 to 180) at ${longitude}`;
  }
